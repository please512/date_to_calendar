export function toCalendar(el,options){
    var calendar_table = {
         ...
         * @param lunar Year
         * @return Number
         * @eg:var count = calendar_table.lYearDays(1987) ;//count=387
         */
        lYearDays: function(y) {
            var i, sum = 348;
            for (i = 0x8000; i > 0x8; i >>= 1) { sum += (calendar_table.lunarInfo[y - 1900] & i) ? 1 : 0; }
            return (sum + calendar_table.leapDays(y));
        },
         ...
         * @param lunar Year
         * @return Number (0-12)
         * @eg:var leapMonth = calendar_table.leapMonth(1987) ;//leapMonth=6
         */
        leapMonth: function(y) { //闰字编码 \u95f0
            return (calendar_table.lunarInfo[y - 1900] & 0xf);
        },
         ...
         * @eg:var leapMonthDay = calendar_table.leapDays(1987) ;//leapMonthDay=29
         */
        leapDays: function(y) {
            if (calendar_table.leapMonth(y)) {
                return ((calendar_table.lunarInfo[y - 1900] & 0x10000) ? 30 : 29);
            }
            return (0);
        },
         ...
         * @eg:var MonthDay = calendar_table.monthDays(1987,9) ;//MonthDay=29
         */
        monthDays: function(y, m) {
            if (m > 12 || m < 1) { return -1 } //月份参数从1至12，参数错误返回-1
            return ((calendar_table.lunarInfo[y - 1900] & (0x10000 >> m)) ? 30 : 29);
        },
         ...
         * @eg:var solarMonthDay = calendar_table.leapDays(1987) ;//solarMonthDay=30
         */
        solarDays: function(y, m) {
            ...
            if (ms == 1) { //2月份的闰平规律测算后确认返回28或29
                ...
                return (calendar_table.solarMonth[ms]);
            }
        },
        ...
        toGanZhiYear: function(lYear) {
            ...
            return calendar_table.Gan[ganKey - 1] + calendar_table.Zhi[zhiKey - 1];
        ...
        toGanZhi: function(offset) {
            return calendar_table.Gan[offset % 10] + calendar_table.Zhi[offset % 12];
        },
         ...
         * @return day Number
         * @eg:var _24 = calendar_table.getTerm(1987,3) ;//_24=4;意即1987年2月4日立春
         */
        getTerm: function(y, n) {
            if (y < 1900 || y > 2100) { return -1; }
            if (n < 1 || n > 24) { return -1; }
            var _table = calendar_table.sTermInfo[y - 1900];
            var _info = [
         ...
         * @param lunar month
         * @return Cn string
         * @eg:var cnMonth = calendar_table.toChinaMonth(12) ;//cnMonth='腊月'
         */
        toChinaMonth: function(m) { // 月 => \u6708
            if (m > 12 || m < 1) { return -1 } //若参数错误 返回-1
            var s = calendar_table.nStr3[m - 1];
            s += "\u6708"; //加上月字
            return s;
        },
         ...
         * @param lunar day
         * @return Cn string
         * @eg:var cnDay = calendar_table.toChinaDay(21) ;//cnMonth='廿一'
         */
        toChinaDay: function(d) { //日 => \u65e5
            ...
            switch (d) {
                ...
                case 30:
                    ...
                    break;
                default:
                    s = calendar_table.nStr2[Math.floor(d / 10)];
                    s += calendar_table.nStr1[d % 10];
            }
            return (s);
        },
         ...
         * @param y year
         * @return Cn string
         * @eg:var animal = calendar_table.getAnimal(1987) ;//animal='兔'
         */
        getAnimal: function(y) {
            return calendar_table.Animals[(y - 4) % 12]
        },
         ...
         * @param d  solar day
         * @return JSON object
         * @eg:console.log(calendar_table.solar2lunar(1987,11,01));
         */
        solar2lunar: function(y, m, d) { //参数区间1900.1.31~2100.12.31
            ...
            for (i = 1900; i < 2101 && offset > 0; i++) {
                temp = calendar_table.lYearDays(i);
                offset -= temp;
            }
            if (offset < 0) {
                offset += temp;
            ...
            var nWeek = objDate.getDay(),
                cWeek = calendar_table.nStr1[nWeek];
            //数字表示周几顺应天朝周一开始的惯例
            if (nWeek == 0) {
                nWeek = 7;
            }
            //农历年
            var year = i;
            var leap = calendar_table.leapMonth(i); //闰哪个月
            var isLeap = false;
            ...
            for (i = 1; i < 13 && offset > 0; i++) {
                ...
                if (leap > 0 && i == (leap + 1) && isLeap == false) {
                    --i;
                    isLeap = true;
                    temp = calendar_table.leapDays(year); //计算农历闰月天数
                } else {
                    temp = calendar_table.monthDays(year, i); //计算农历普通月天数
                }
                //解除闰月
            ...
            //天干地支处理
            var sm = m - 1;
            var gzY = calendar_table.toGanZhiYear(year);

            // 当月的两个节气
            // bugfix-2017-7-24 11:03:38 use lunar Year Param `y` Not `year`
            var firstNode = calendar_table.getTerm(y, (m * 2 - 1)); //返回当月「节」为几日开始
            var secondNode = calendar_table.getTerm(y, (m * 2)); //返回当月「节」为几日开始

            // 依据12节气修正干支月
            var gzM = calendar_table.toGanZhi((y - 1900) * 12 + m + 11);
            if (d >= firstNode) {
                gzM = calendar_table.toGanZhi((y - 1900) * 12 + m + 12);
            }
            ...
            if (firstNode == d) {
                isTerm = true;
                Term = calendar_table.solarTerm[m * 2 - 2];
            }
            if (secondNode == d) {
                isTerm = true;
                Term = calendar_table.solarTerm[m * 2 - 1];
            }
            //日柱 当月一日与 1900/1/1 相差天数
            var dayCyclical = Date.UTC(y, sm, 1, 0, 0, 0, 0) / 86400000 + 25567 + 10;
            var gzD = calendar_table.toGanZhi(dayCyclical + d - 1);
            //该日期所属的星座
            var astro = calendar_table.toAstro(m, d);
            return `${calendar_table.toChinaDay(day)=='初一'?(isLeap ? "\u95f0" : '') + calendar_table.toChinaMonth(month):calendar_table.toChinaDay(day)} ${Term?Term:''} ${config.isShowAstro?astro:''}`
            // console.log(calendar_table.getAnimal(year))
            // console.log((isLeap ? "\u95f0" : '') + calendar_table.toChinaMonth(month))
            // console.log(gzY)
            // console.log(gzM)
            // console.log(gzD)
            // console.log(isLeap)
            ...
            // console.log(Term)
            // console.log(astro)
            // return `${gzY}（${calendar_table.getAnimal(year)}） ${(isLeap ? "\u95f0" : '') + calendar_table.toChinaMonth(month)}${calendar_table.toChinaDay(day)} ${Term?Term:''} ${astro}`
            // return { 'lYear': year, 'lMonth': month, 'lDay': day, 'Animal': calendar_table.getAnimal(year), 'IMonthCn': (isLeap ? "\u95f0" : '') + calendar_table.toChinaMonth(month), 'IDayCn': calendar_table.toChinaDay(day), 'cYear': y, 'cMonth': m, 'cDay': d, 'gzYear': gzY, 'gzMonth': gzM, 'gzDay': gzD, 'isToday': isToday, 'isLeap': isLeap, 'nWeek': nWeek, 'ncWeek': "\u661f\u671f" + cWeek, 'isTerm': isTerm, 'Term': Term, 'astro': astro };
        },
         ...
         * @return JSON object
         * @eg:console.log(calendar_table.lunar2solar(1987,9,10));
         */
        lunar2solar: function(y, m, d, isLeapMonth) { //参数区间1900.1.31~2100.12.1
            var isLeapMonth = !!isLeapMonth;
            var leapOffset = 0;
            var leapMonth = calendar_table.leapMonth(y);
            var leapDay = calendar_table.leapDays(y);
            if (isLeapMonth && (leapMonth != m)) { return -1; } //传参要求计算该闰月公历 但该年得出的闰月与传参的月份并不同
            if (y == 2100 && m == 12 && d > 1 || y == 1900 && m == 1 && d < 31) { return -1; } //超出了最大极限值 
            var day = calendar_table.monthDays(y, m);
            var _day = day;
            //bugFix 2016-9-25 
            //if month is leap, _day use leapDays method 
            if (isLeapMonth) {
                _day = calendar_table.leapDays(y, m);
            }
            ...
            for (var i = 1900; i < y; i++) {
                offset += calendar_table.lYearDays(i);
            }
            var leap = 0,
                isAdd = false;
            for (var i = 1; i < m; i++) {
                leap = calendar_table.leapMonth(y);
                if (!isAdd) { //处理闰月
                    if (leap <= i && leap > 0) {
                        offset += calendar_table.leapDays(y);
                        isAdd = true;
                    }
                }
                offset += calendar_table.monthDays(y, i);
            }
            ...
            return calendar_table.solar2lunar(cY, cM, cD);
        },
    ...
    let config={
        backgroundDict:options.backgroundDict?options.backgroundDict:{
            '1':'#ca8687',
            '2':'#da765b',
            '3':'#b7704f',
            '4':'#deab8a',
        ...
        isShowChinese:options.isShowChinese
    }
    ...
    if(start_num!=end_num){//判断不是同一天
        ...
        do{ 
            ...
            center_todo.push({
                ...
                date_nong:calendar_table.parseDate(formatDate(collectList[count])),
                background:config.backgroundDict[new Date(collectList[count]).getMonth()+1],
            })
            count++
        ...
        calendar.push({
            ...
            date_nong:calendar_table.parseDate(formatDate(Date.parse(new Date(start)))),
            background:config.backgroundDict[new Date(start).getMonth()+1]
        })
    ...
    for(let i=0;i<calendar.length;i++){
        ...
        for(let j=0;j<calendar[i].length;j++){
            otd+=`<td style="cursor:pointer;color:#fff;background:${calendar[i][j].background};border:${calendar[i][j].background?'1px #EEE9E9 solid':''};width:${config.tdWidth}px;height: ${config.tdHeight}px;text-align: center;">`+
                    `<p style='font-weight:600;margin:5px 0;margin-top:10px;'>${calendar[i][j].date}</p>`+
                    `<p style='font-weight:600;font-size:12px;color:#281f1d;margin:5px 0;margin-bottom:10px;display:${config.isShowChinese?'block':'none'}'>${calendar[i][j].date_nong}</p>`+
                `</td>`
        }
        otr+=`<tr>`+
            otd+
